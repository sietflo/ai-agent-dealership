...
def list_Salesmans(db:Session = Depends(get_db)):
...
def create_salesman(Salesman_data: SalesmanCreate, db: Session=Depends(get_db)):
...
def update_salesman(Salesman_id: int, Salesman_data: SalesmanUpdate, db: Session = Depends(get_db)):
