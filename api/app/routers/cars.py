...
# Prefix sets the base path for all routes in this file
router = APIRouter(prefix="/cars", tags=["Cars"])


# 1. GET ALL CARS (with optional status filter)
@router.get("/", response_model=List[CarResponse])
def list_cars(status: str = None, db: Session = Depends(get_db)):
    query = db.query(Car)
    if status:
        query = query.filter(Car.status == status.upper())
    return query.all()
